...
def sales_kiosk(a):
    W, D, H = 240, 200, 300
    a.box((W, D, 10), at=(0, 0, 5), col=C.GREY_DARK, bevel=3)
    for s in (-1, 1):
        a.box((W, 14, H), at=(0, s * (D / 2 - 7), 10 + H / 2), col=C.CREAM, bevel=4)
    a.box((14, D, H), at=(W / 2 - 7, 0, 10 + H / 2), col=C.CREAM, bevel=4)
    a.box((14, D, 100), at=(-W / 2 + 7, 0, 60), col=C.CREAM, bevel=4)
    a.box((14, D, 70), at=(-W / 2 + 7, 0, 10 + H - 35), col=C.CREAM, bevel=4)
    for sx in (-1, 1):
        for sy in (-1, 1):
            a.box((20, 20, H + 4), at=(sx * (W / 2 - 6), sy * (D / 2 - 6), 12 + H / 2), col=C.TEAL, bevel=5)
    a.box((W + 30, D + 30, 16), at=(0, 0, 18 + H), col=C.TEAL_DARK, bevel=5)
    a.box((W + 10, D + 10, 12), at=(0, 0, 32 + H), col=C.CORAL, bevel=4)
    # service window + counter
    a.box((6, D - 30, 130), at=(-W / 2 + 4, 0, 175), col=C.GLASS, mat="glass")
    a.box((40, D - 20, 8), at=(-W / 2 - 10, 0, 112), col=C.WOOD, bevel=3)
    for k in range(3):
    ...
    a.box((110, 6, 90), at=(20, -D / 2 - 1, 170), col=C.GLASS, mat="glass")
    a.box((110, 30, 6), at=(20, -D / 2 - 14, 125), col=C.WOOD, bevel=2)
    # awning over the service window
    n = 8
    for k in range(n):
        a.box((70, D / n + 1, 6), at=(-W / 2 - 34, -D / 2 + (k + 0.5) * D / n, 262), rot=(18, 0, 0), col=C.CORAL if k % 2 == 0 else C.WHITE, bevel=1.5)
    # brochure rack outside
    a.box((30, 50, 110), at=(-W / 2 - 20, D / 2 + 40, 55), col=C.CHROME, bevel=3, rough=0.25)
    for k in range(3):
        a.box((6, 44, 26), at=(-W / 2 - 36, D / 2 + 40, 40 + k * 32), rot=(0, 0, 0), col=[C.CORAL, C.TEAL, C.YELLOW][k], bevel=1)
    # roof sign
    ...
    for s in (-1, 1):
        a.cyl(1.5, 80, at=(W / 2, s * (D / 2), H + 80), col=C.GREY, sides=6)
        a.prism((1.5, 30, 20), at=(W / 2, s * (D / 2) + 15, H + 110), rot=(0, 0, 90), col=C.MAGENTA if s < 0 else C.CYAN)
