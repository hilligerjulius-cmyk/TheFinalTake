...
void UFTTitleMenuWidget::NativeOnInitialized()
{
	...
	// ---- career slots: every slot is a separate studio "world" with its own money, purchases and films
	{
		UVerticalBox* CV = T->ConstructWidget<UVerticalBox>();
		CareerHeader = Text(T, LOCTEXT("CareerHeader", "CHOOSE YOUR STUDIO"), 40, Cream, TEXT("Black"), true);
		AddV(CV, CareerHeader, FMargin(0.f, 0.f, 0.f, 4.f));
		AddV(CV, Text(T, LOCTEXT("CareerSub", "Each slot is its own studio: money, purchases, placed sets, cars and released films are saved automatically."), 15, Amber, TEXT("Bold")), FMargin(0.f, 0.f, 0.f, 16.f));
		for (int32 i = 0; i < 3; ++i)
		{
			UBorder* Card = Box(T, Paper, 14.f, FMargin(18.f, 12.f), Ink, 3.f);
			UHorizontalBox* Row = T->ConstructWidget<UHorizontalBox>();
			UVerticalBox* Info = T->ConstructWidget<UVerticalBox>();
			UTextBlock* TitleLine = Text(T, FText::GetEmpty(), 22, Ink, TEXT("Black"));
			UTextBlock* Detail = Text(T, FText::GetEmpty(), 14, Muted, TEXT("Bold"));
			Detail->SetAutoWrapText(false);
			Detail->SetWrapTextAt(330.f);
			AddV(Info, TitleLine);
			AddV(Info, Detail, FMargin(0.f, 2.f, 0.f, 0.f));
			AddH(Row, Sized(T, Info, 340.f), FMargin(0.f, 0.f, 12.f, 0.f), false, 1);
			TObjectPtr<UTextBlock> PlayLabel;
			UButton* PlayBtn = Button(T, LOCTEXT("SlotPlay", "PLAY"), Coral, 20, &PlayLabel);
			AddH(Row, Sized(T, PlayBtn, 150.f), FMargin(0.f, 0.f, 8.f, 0.f), false, 1);
			TObjectPtr<UTextBlock> DelLabel;
			UButton* DelBtn = Button(T, LOCTEXT("SlotDelete", "DELETE"), PaperDark, 14, &DelLabel);
			AddH(Row, Sized(T, DelBtn, 130.f), FMargin(0.f), false, 1);
			Card->SetContent(Row);
			AddV(CV, Card, FMargin(0.f, 6.f));
			SlotTitles.Add(TitleLine);
			SlotDetails.Add(Detail);
			SlotPlayLabels.Add(PlayLabel);
			SlotDeleteLabels.Add(DelLabel);
			SlotDeleteButtons.Add(DelBtn);
			switch (i)
			{
			case 0: PlayBtn->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleSlot1); DelBtn->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleDelete1); break;
			case 1: PlayBtn->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleSlot2); DelBtn->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleDelete2); break;
			default: PlayBtn->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleSlot3); DelBtn->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleDelete3); break;
			}
		}
		UButton* Back = Button(T, LOCTEXT("CareerBack", "BACK"), Cream, 18);
		Back->OnClicked.AddDynamic(this, &UFTTitleMenuWidget::HandleCareerBack);
		AddV(CV, Sized(T, Back, 200.f), FMargin(0.f, 14.f, 0.f, 0.f));
		CareerPanel = CV;
		Place(Root, CV, FAnchors(0.f, 0.5f), FVector2D(0.f, 0.5f), FVector2D(90.f, 0.f));
		CareerPanel->SetVisibility(ESlateVisibility::Collapsed);
	}
}

void UFTTitleMenuWidget::ShowCareer(bool bShow, bool bHost)
{
	bCareerHost = bHost;
	DeleteArmed = 0;
	if (bShow)
	...
	CareerPanel->SetVisibility(bShow ? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	MainColumn->SetVisibility(bShow ? ESlateVisibility::Collapsed : ESlateVisibility::Visible);
}

void UFTTitleMenuWidget::RefreshSlots()
{
	CareerHeader->SetText(bCareerHost ? LOCTEXT("CareerHeaderHost", "HOST WHICH STUDIO?") : LOCTEXT("CareerHeaderPlay", "CHOOSE YOUR STUDIO"));
	for (int32 i = 0; i < SlotTitles.Num(); ++i)
	{
		const FFTCareerSlotInfo Info = UFTCareerSave::Peek(i + 1);
		if (!Info.bExists)
		{
			SlotTitles[i]->SetText(FText::Format(LOCTEXT("SlotEmpty", "SLOT {0}  -  empty"), FText::AsNumber(i + 1)));
			SlotDetails[i]->SetText(LOCTEXT("SlotEmptyD", "Start a brand new studio with $5,000 and a studio van."));
			SlotPlayLabels[i]->SetText(LOCTEXT("SlotNew", "NEW"));
			SlotDeleteButtons[i]->SetIsEnabled(false);
		}
		else if (Info.bCorrupt)
		{
			SlotTitles[i]->SetText(FText::Format(LOCTEXT("SlotBad", "SLOT {0}  -  damaged"), FText::AsNumber(i + 1)));
			SlotDetails[i]->SetText(LOCTEXT("SlotBadD", "The save file is damaged. Playing restores the backup or starts fresh."));
			SlotPlayLabels[i]->SetText(LOCTEXT("SlotRepair", "REPAIR"));
			SlotDeleteButtons[i]->SetIsEnabled(true);
		}
		else
		{
			SlotTitles[i]->SetText(FText::Format(LOCTEXT("SlotUsed", "SLOT {0}  -  {1}"), FText::AsNumber(i + 1), FText::FromString(Info.StudioName)));
			SlotDetails[i]->SetText(FText::Format(LOCTEXT("SlotUsedD", "{0}  -  {1} film(s)  -  {2} stage(s)  -  best {3}\nsaved {4}"),
				FText::FromString(FFTEconomy::MoneyString(Info.Money)), FText::AsNumber(Info.FilmsReleased), FText::AsNumber(Info.StagesUnlocked),
				FText::FromString(FFTEconomy::MoneyString(Info.BestRevenue)), FText::AsDateTime(Info.LastSaved)));
			SlotPlayLabels[i]->SetText(LOCTEXT("SlotContinue", "CONTINUE"));
			SlotDeleteButtons[i]->SetIsEnabled(true);
		}
		SlotDeleteLabels[i]->SetText(DeleteArmed == i + 1 ? LOCTEXT("SlotConfirm", "SURE?") : LOCTEXT("SlotDel", "DELETE"));
	}
}

void UFTTitleMenuWidget::PlaySlot(int32 SlotIndex)
{
	FTAudio::Play2D(this, EFTSound::Clapper, 1.f);
	if (AFTPlayerController* P = PC())
	{
		P->StartDemo(bCareerHost, SlotIndex);
	}
}

void UFTTitleMenuWidget::DeleteSlot(int32 SlotIndex)
{
	Click(this);
	if (DeleteArmed != SlotIndex)
	{
		DeleteArmed = SlotIndex; // first click arms, second click deletes
		RefreshSlots();
		return;
	}
	UFTCareerSave::DeleteSlot(SlotIndex);
	DeleteArmed = 0;
	RefreshSlots();
}

void UFTTitleMenuWidget::HandleSlot1() { PlaySlot(1); }
void UFTTitleMenuWidget::HandleSlot2() { PlaySlot(2); }
void UFTTitleMenuWidget::HandleSlot3() { PlaySlot(3); }
void UFTTitleMenuWidget::HandleDelete1() { DeleteSlot(1); }
void UFTTitleMenuWidget::HandleDelete2() { DeleteSlot(2); }
void UFTTitleMenuWidget::HandleDelete3() { DeleteSlot(3); }

void UFTTitleMenuWidget::HandleCareerBack()
{
	Click(this);
	ShowCareer(false, false);
}
...
{
	Click(this);
	ShowCareer(true, false);
}

void UFTTitleMenuWidget::HandleHost()
{
	Click(this);
	ShowCareer(true, true);
}
...
void UFTPauseMenuWidget::NativeOnInitialized()
{
	...
	UButton* Leave = Button(T, LOCTEXT("Leave", "LEAVE TO TITLE (CAREER IS SAVED)"), PaperDark, 18);
	...
	CareerText = Text(T, FText::GetEmpty(), 15, Cream, TEXT("Bold"), true);
	CareerText->SetJustification(ETextJustify::Center);
	AddV(V, CareerText, FMargin(0.f, 14.f, 0.f, 4.f), 1);
	ResetCareerButton = Button(T, LOCTEXT("ResetCareer", "RESET CAREER"), Coral, 14, &ResetCareerLabel);
	ResetCareerButton->OnClicked.AddDynamic(this, &UFTPauseMenuWidget::HandleResetCareer);
	AddV(V, Sized(T, ResetCareerButton, 300.f), FMargin(0.f, 2.f), 1);
...
void UFTPauseMenuWidget::Refresh()
{
	...
	if (GS)
	{
		CareerText->SetText(FText::Format(LOCTEXT("CareerLine", "{0}  (slot {1})  -  {2}  -  {3} film(s) released\nThe career saves automatically after every purchase and premiere."),
			FText::FromString(GS->StudioName), FText::AsNumber(GS->CareerSlot), FText::FromString(FFTEconomy::MoneyString(GS->StudioMoney)), FText::AsNumber(GS->CareerFilmsTotal)));
	}
	const bool bQuiet = GS && !GS->IsShootActive() && GS->ShootPhase != EFTShootPhase::Premiere;
	bResetArmed = false;
	ResetCareerButton->SetVisibility(bHost ? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	ResetCareerButton->SetIsEnabled(bQuiet);
	ResetCareerLabel->SetText(bQuiet ? LOCTEXT("ResetCareer2", "RESET CAREER") : LOCTEXT("ResetBusyLbl", "RESET CAREER (NOT DURING A SHOOT)"));
	Settings->SetVisibility(ESlateVisibility::Collapsed);
	MainColumn->SetVisibility(ESlateVisibility::Visible);
...
void UFTPauseMenuWidget::HandleResetCareer()
{
	Click(this);
	if (!bResetArmed)
	{
		bResetArmed = true; // first click arms, second click wipes the slot
		ResetCareerLabel->SetText(LOCTEXT("ResetSure", "REALLY DELETE ALL PROGRESS?"));
		return;
	}
	bResetArmed = false;
	if (AFTPlayerController* P = PC())
...
void UFTResultsWidget::NativeOnInitialized()
{
	...
	UCanvasPanel* Root = MakeRoot(T);
	Fullscreen(Root, Swatch(T, Shade, FVector2D(10.f)));
	UBorder* C = Box(T, Paper, 18.f, FMargin(30.f, 22.f), Ink, 4.f);
	...
	LeftColumn = T->ConstructWidget<UVerticalBox>();
	RightColumn = T->ConstructWidget<UVerticalBox>();
	AddH(Columns, Sized(T, LeftColumn, 600.f), FMargin(0.f, 0.f, 26.f, 0.f), false, 0);
	AddH(Columns, Sized(T, RightColumn, 480.f), FMargin(0.f), false, 0);
	AddV(Outer, Columns, FMargin(0.f));
	UHorizontalBox* Buttons = T->ConstructWidget<UHorizontalBox>();
	RetryButton = Button(T, LOCTEXT("Retry", "NEXT SHOOT (BACK TO LOBBY)"), Coral, 22);
	...
	Card = C;
	Place(Root, Sized(T, C, 1180.f), FAnchors(0.5f, 0.5f), FVector2D(0.5f, 0.5f), FVector2D::ZeroVector);
}
...
void UFTResultsWidget::Refresh()
{
	...
	const FFTReleaseReport& Rep = GS->LastRelease;
	AddV(Content, Text(T, bFailed ? LOCTEXT("BlackoutT", "BLACKOUT!") : LOCTEXT("BoxOfficeT", "BOX OFFICE"), 42, bFailed ? Red : Ink, TEXT("Black")), FMargin(0.f), 1);
	if (bFailed)
	{
		AddV(Content, Text(T, GS->FailReason, 18, CoralDarkText(), TEXT("Bold")), FMargin(0.f, 0.f, 0.f, 4.f), 1);
		AddV(Content, Text(T, LOCTEXT("NoRelease", "The film never premiered - no tickets sold, no money earned. Try again tonight!"), 16, Muted, TEXT("Bold")), FMargin(0.f, 0.f, 0.f, 10.f), 1);
	}
	if (Film)
	{
		UHorizontalBox* TitleRow = T->ConstructWidget<UHorizontalBox>();
		AddH(TitleRow, Text(T, Film->Title, 22, TealDark, TEXT("Black")), FMargin(0.f, 0.f, 14.f, 0.f), false, 1);
		if (Rep.bValid)
		{
			for (int32 s = 0; s < 5; ++s)
			{
				AddH(TitleRow, Swatch(T, s < Rep.Stars ? Amber : PaperDark, FVector2D(20.f, 20.f), 10.f), FMargin(2.f, 0.f), false, 1);
			}
		}
		AddV(Content, TitleRow, FMargin(0.f, 0.f, 0.f, 10.f), 1);
	}

	// ---- left: the takes and how the audience was built
	auto Header = [&](UVerticalBox* Col, const FText& Label)
	{
		AddV(Col, Text(T, Label, 15, TealDark, TEXT("Black")), FMargin(0.f, 6.f, 0.f, 2.f));
	};
	auto Line = [&](UVerticalBox* Col, const FText& Label, const FText& Value, int32 Size, const FLinearColor& ValueColor)
	{
		UHorizontalBox* Row = T->ConstructWidget<UHorizontalBox>();
		UTextBlock* L = Text(T, Label, Size, Ink, TEXT("Bold"));
		L->SetAutoWrapText(false);
		L->SetWrapTextAt(430.f);
		AddH(Row, L, FMargin(0.f), true);
		AddH(Row, Text(T, Value, Size, ValueColor, TEXT("Black")), FMargin(10.f, 0.f, 0.f, 0.f));
		AddV(Col, Row, FMargin(0.f, 1.f), 3);
	};
	if (Film)
	{
		Header(LeftColumn, LOCTEXT("HTakes", "THE TAKES"));
		for (int32 i = 0; i < Film->Scenes.Num(); ++i)
		{
			UHorizontalBox* Row = T->ConstructWidget<UHorizontalBox>();
			AddH(Row, Text(T, Film->Scenes[i].Title, 16, Ink, TEXT("Bold")), FMargin(0.f), true);
			const FFTTakeResult* Best = GS->GetBestTake(i);
			UBorder* Stamp = Box(T, Best ? RatingColor(Best->Rating) : PaperDark, 8.f, FMargin(8.f, 1.f), Ink, 2.f);
			Stamp->SetContent(Text(T, Best ? RatingName(Best->Rating) : LOCTEXT("NotShot", "NOT SHOT"), 12, Ink, TEXT("Black")));
			Stamp->SetRenderTransformAngle(-4.f);
			AddH(Row, Stamp, FMargin(8.f, 0.f));
			AddH(Row, Sized(T, Text(T, Best ? FText::Format(LOCTEXT("Pts", "{0} pts"), FText::AsNumber(Best->Score)) : FText::FromString(TEXT("-")), 16, Ink, TEXT("Black")), 80.f));
			AddV(LeftColumn, Row, FMargin(0.f, 2.f), 3);
		}
	}
	if (Rep.bValid)
	{
		Header(LeftColumn, LOCTEXT("HAudience", "HOW THE AUDIENCE CAME TOGETHER"));
		for (const FFTReleaseLine& L : Rep.Lines)
		{
			const FString V = (L.Audience >= 0 ? TEXT("+") : TEXT("")) + FFTEconomy::NumberString(L.Audience);
			Line(LeftColumn, L.Label, FText::FromString(V), 14, L.Audience >= 0 ? TealDark : CoralDarkText());
		}
		AddV(LeftColumn, Swatch(T, PaperDark, FVector2D(10.f, 3.f)), FMargin(0.f, 6.f), 3);
		Line(LeftColumn, LOCTEXT("LTotal", "Viewers"), FText::FromString(FFTEconomy::NumberString(Rep.Audience)), 18, Ink);
		Line(LeftColumn, FText::Format(LOCTEXT("LTicket", "Tickets at {0}, studio keeps half"), FText::FromString(FFTEconomy::MoneyString(Rep.TicketPrice))),
			FText::FromString(FFTEconomy::MoneyString(Rep.Revenue)), 18, TealDark);
	}

	// ---- right: verdict, critics and the studio account
	if (Rep.bValid)
	{
		UBorder* TierStamp = Box(T, Rep.Tier == EFTBoxOfficeTier::Flop ? Coral : (Rep.Tier == EFTBoxOfficeTier::CultClassic ? Magenta : Teal), 12.f, FMargin(18.f, 6.f), Ink, 4.f);
		TierStamp->SetContent(Text(T, FFTEconomy::TierName(Rep.Tier), 32, Cream, TEXT("Black"), true));
		TierStamp->SetRenderTransformAngle(-3.f);
		AddV(RightColumn, TierStamp, FMargin(0.f, 6.f, 0.f, 6.f), 1);
		AddV(RightColumn, Text(T, FText::Format(LOCTEXT("Rank", "Box-office rank #{0} of {1} studio releases"), FText::AsNumber(Rep.Rank), FText::AsNumber(Rep.RankOf)), 16, Ink, TEXT("Black")), FMargin(0.f, 0.f, 0.f, 8.f), 1);
		Header(RightColumn, LOCTEXT("HCritics", "THE CRITICS"));
		for (int32 i = 0; i < Rep.Reviews.Num(); ++i)
		{
			UBorder* Quote = Box(T, Cream, 10.f, FMargin(12.f, 6.f), PaperDark, 2.f);
			UVerticalBox* QV = T->ConstructWidget<UVerticalBox>();
			UTextBlock* QT = Text(T, Rep.Reviews[i], 14, Ink, TEXT("Italic"));
			QT->SetAutoWrapText(false);
			QT->SetWrapTextAt(430.f);
			AddV(QV, QT);
			AddV(QV, Text(T, Rep.ReviewSources.IsValidIndex(i) ? FText::Format(LOCTEXT("Src", "- {0}"), Rep.ReviewSources[i]) : FText::GetEmpty(), 12, TealDark, TEXT("Bold")), FMargin(0.f, 2.f, 0.f, 0.f), 2);
			Quote->SetContent(QV);
			AddV(RightColumn, Quote, FMargin(0.f, 3.f), 3);
		}
		UBorder* Money = Box(T, Ink, 12.f, FMargin(16.f, 8.f), Cream, 3.f);
		UVerticalBox* MV = T->ConstructWidget<UVerticalBox>();
		AddV(MV, Text(T, FText::Format(LOCTEXT("Earned", "STUDIO EARNS {0}"), FText::FromString(FFTEconomy::MoneyString(Rep.Revenue))), 22, Yellow, TEXT("Black")), FMargin(0.f), 1);
		AddV(MV, Text(T, FText::Format(LOCTEXT("Balance", "Team balance now {0}"), FText::FromString(FFTEconomy::MoneyString(GS->StudioMoney))), 15, Cream, TEXT("Bold")), FMargin(0.f), 1);
		Money->SetContent(MV);
		AddV(RightColumn, Money, FMargin(0.f, 10.f, 0.f, 0.f), 3);
	}
	else if (!bFailed)
	{
		AddV(RightColumn, Text(T, LOCTEXT("Counting", "Counting ticket stubs..."), 18, Muted, TEXT("Bold")), FMargin(0.f, 20.f), 1);
	}
	...
	AddV(RightColumn, Text(T, FText::Format(LOCTEXT("CreditsStudio", "A {0} production"), FText::FromString(GS->StudioName.IsEmpty() ? FString(TEXT("Final Take")) : GS->StudioName)), 14, Ink, TEXT("Black")), FMargin(0.f, 10.f, 0.f, 0.f), 1);
	AddV(RightColumn, Text(T, FText::FromString(Names), 13, TealDark, TEXT("Bold")), FMargin(0.f), 1);
...
void UFTPremiereWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	if (Card != LastCard)
	{
		...
		{
			// box office card: the release verdict the whole cinema sees
			const FFTReleaseReport& Rep = GS->LastRelease;
			Backdrop->SetBrushColor(Film->PosterPrimary * 0.35f + FLinearColor(0.f, 0.f, 0.f, 1.f));
			if (Rep.bValid)
			{
				BigText->SetText(FText::Format(LOCTEXT("PremViewers", "{0} VIEWERS"), FText::FromString(FFTEconomy::NumberString(Rep.Audience))));
				SubText->SetText(FText::Format(LOCTEXT("PremVerdict", "{0}  -  box office #{1}  -  {2} for the studio\n\"{3}\""),
					FFTEconomy::TierName(Rep.Tier), FText::AsNumber(Rep.Rank), FText::FromString(FFTEconomy::MoneyString(Rep.Revenue)),
					Rep.Reviews.Num() > 0 ? Rep.Reviews[0] : FText::GetEmpty()));
				Stamp->SetVisibility(ESlateVisibility::HitTestInvisible);
				Stamp->SetBrush(Round(Rep.Tier == EFTBoxOfficeTier::Flop ? FTColors::Coral : FTColors::Teal, 16.f, Cream, 6.f));
				StampText->SetText(FText::Format(LOCTEXT("PremStars", "{0} / 5"), FText::AsNumber(Rep.Stars)));
				Stamp->SetRenderScale(FVector2D(1.f));
				Stamp->SetRenderOpacity(1.f);
				FTAudio::Play2D(this, Rep.Stars >= 4 ? EFTSound::ApplauseBig : (Rep.Stars >= 2 ? EFTSound::ApplauseSmall : EFTSound::Groan), 0.9f);
			}
			else
			{
				BigText->SetText(LOCTEXT("PremCounting", "COUNTING TICKETS..."));
				SubText->SetText(FText::GetEmpty());
			}
		}
	}
